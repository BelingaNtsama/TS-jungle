...
const AiChat = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(async () => {
        const response = await axiosInstance.post('/Agent', { message: input });
        setIsTyping(false);
                          ...
                          {message.role === 'assistant' ? (
                            <ReactMarkdown 
                              ...
                              components={{
                                ...
                                blockquote: ({node, ...props}) => <blockquote className="border-l-4 border-green-300 pl-3 my-2 text-gray-600" {...props} />,
                                code: ({node, ...props}) => <code className="bg-gray-100 px-1 py-0.5 rounded text-sm font-mono" {...props} />,
                                pre: ({node, ...props}) => <pre className="bg-gray-800 text-gray-100 p-3 rounded overflow-x-auto my-2 text-sm" {...props} />,
                                table: ({node, ...props}) => <table className="border-collapse border border-gray-300 my-2" {...props} />,
                                th: ({node, ...props}) => <th className="border border-gray-300 px-3 py-1 bg-gray-100 font-semibold" {...props} />,
